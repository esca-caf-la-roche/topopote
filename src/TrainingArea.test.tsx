...
describe('TrainingArea', () => {
  ...
  it('affiche le formulaire simple uniquement au profil autorisé', async () => {
    ...
    expect(screen.queryByLabelText('Doigts')).toBeNull()
    const painGroup = screen.getByRole('group', { name: 'Douleurs' })
    expect(painGroup.classList.contains('training-pain-panel')).toBe(true)
  ...
  it('sépare les essais, permet la modification et filtre les suggestions par falaise', async () => {
    const sessions = [
      { id: 'session-2', user_id: 'user-1', date_seance: '2026-08-18', type_lieu: 'exterieur', falaise: 'Ablon', duree_minutes: 60, effort_percu: 7, douleur: 0, created_at: '2026-08-18T12:00:00Z' },
    ...
    expect(editPainGroup.classList.contains('training-pain-panel')).toBe(true)
    fireEvent.click(within(editPainGroup).getByRole('radio', { name: 'Oui' }))
    expect(within(editPainGroup).getByRole('spinbutton', { name: 'Douleur (1–10)' })).toBeTruthy()
    expect(within(editPainGroup).getByLabelText('Partie du corps douloureuse')).toBeTruthy()
    expect(within(editPainGroup).getByLabelText('Type de douleur')).toBeTruthy()
  ...
  it('additionne escalade et autres activités tout en signalant un total partiel', async () => {
    ...
    const activities = [{ id: 'activity-1', user_id: 'user-1', date_activite: '2026-08-20', type_activite: 'poutre', duree_minutes: 30, effort_percu: 6, douleur: 4, created_at: '2026-08-20T12:00:00Z' }]
    from.mockImplementation((table: string) => queryResult(table === 'seances_entrainement' ? sessions : table === 'activites_charge' ? activities : []))
    render(<TrainingArea user={{ id: 'user-1' } as never} isAdmin={false} isOpener={false} sharesActivity={false} hasTrainingAccess authLoading={false} routes={[]} grades={[]} onSignOut={async () => undefined} />)
    expect(await screen.findByText('600 UA')).toBeTruthy()
    expect(screen.getByText('1 activité à compléter · total partiel')).toBeTruthy()
    const tuesdaySession = screen.getByLabelText('mardi 18/08/2026 : 420 UA')
    const thursdaySession = screen.getByLabelText('jeudi 20/08/2026 : 180 UA, douleur 4 sur 10')
    expect(tuesdaySession.style.flexGrow).toBe('420')
    expect(thursdaySession.style.flexGrow).toBe('180')
    expect(within(tuesdaySession).getByText('mar')).toBeTruthy()
    expect(within(thursdaySession).getByText('jeu')).toBeTruthy()
    expect(within(thursdaySession).getByLabelText('Douleur 4 sur 10').textContent).toBe('4')
    expect(screen.getByLabelText('Séances à compléter').textContent).toContain('mer ?')
    expect(screen.queryByText('lun')).toBeNull()
    expect(screen.getByText(/Repère en cours :/).closest('p')!.textContent).toContain('0/4 semaines complètes sans douleur déclarée')
    const activityCard = screen.getByText('Douleur 4/10').closest('article')!
    expect(within(activityCard).queryByText(/Doigts/)).toBeNull()
  })
})
