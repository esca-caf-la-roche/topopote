...
Public clients read `saisons`, `zones`, `voies`, `relais`, `couleurs`, and `cotations`. Each route belongs to a season, and each relay belongs to a zone. Only users listed in `administrateurs` can write the topo and its references; practitioners can write only their own profile, logbook and following relations through RLS and narrow RPCs. A database trigger assigns every new route to the active season and rejects the write when no season is active.
...
The topo loads only the current climber's route IDs and styles for card coloring. Community route details are fetched lazily when a connected climber opens a route. Returning from the logbook refreshes this snapshot so profile consent and card colors cannot remain stale. Community sharing consent does not filter the operational route feedback available to openers and administrators; that view includes every recorded response without exposing practitioner identities to openers.
