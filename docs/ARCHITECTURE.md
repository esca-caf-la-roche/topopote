...
Le menu principal est partagé par toutes les pages et dérivé du rôle résolu côté serveur : connexion seule en public, topo, classement, carnet et potes pour un pratiquant, puis topo, classement, carnet, potes et administration pour un administrateur. Les administrateurs réutilisent leur compte Auth existant ; un trigger leur provisionne automatiquement un profil de carnet privé, sans seconde inscription.
...
Le workflow GitHub Actions reconstruit une base Supabase locale et exécute les deux matrices pgTAP avant d’autoriser le déploiement Pages. Il vérifie également la présence des deux secrets Vite avant la compilation, sans journaliser leur contenu. Les actions tierces sont épinglées à des SHA immuables.
